...
export function WeatherBentoCard() {
  ...
  // Narrowed pair for the success branches (TS can't narrow via a bool).
  const ready = !error && !isLoading && current && desc ? { current, desc } : null;
  ...
  return (
    <section className="bento-card area-vaer">
      <div className={`viz ${scene}`}>
        {/* The location picker sits in the hero in every state so a user
         * stuck on the Halden default (or on a fetch error) can still pick
         * a city. */}
        <div className="viz-hero">
          {ready ? (
            <>
              <WeatherScene scene={scene} />
              <div className="cond">{ready.desc.label}</div>
              <div className="temp">{Math.round(ready.current.temperature)}&deg;</div>
              <div className="wind">Vind {wind} m/s</div>
            </>
          ) : (
            <div className="viz-msg">{error ? 'Kunne ikke hente vær.' : 'Laster værdata…'}</div>
          )}
          <WeatherLocationPicker location={location} onSelect={setLocation} />
        </div>
