...
- **Still file-backed** (Phase 4 work in progress): `/api/notes` (lives in the separate `notes_api.py` Flask sidecar on port 5001; it folds into `api.py` when migrated). The file-based POST fallback in `api.py:do_POST` is fully removed since every path now has its own auth-gated handler.
