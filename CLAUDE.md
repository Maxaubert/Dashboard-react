...
- **Cached/integration endpoints (Phase 5)**: `/api/skole`, `/api/wishlist`, `/api/pdf`, `/api/report` are all now auth-gated via `require_auth()`. They still use the global service-account credentials (`CANVAS_TOKEN`, `STEAM_API_KEY`, `ITAD_API_KEY`, `STEAM_ID`) from `/etc/dashboard.env`. The full per-user-encrypted-credentials story (`user_integrations` table + AES-GCM) is deferred until there's a real multi-user need. `/api/news` and `/api/favicon` stay public because they carry no user data.
